...
export const getOverview = async (req, res) => {
  try {
    ...
    const [pendingJobs, completedJobs] = await Promise.all([
      Booking.countDocuments({ status: { $in: ['Pending','Confirmed','Queued'] } }),
    ...
    const upcoming = await Booking.find({ date: { $gte: startStr, $lt: endStr } }, { date: 1 }).lean();
    ...
    res.json({ pendingJobs, completedJobs, next14Days });
  } catch (error) {
    res.status(500).json({ message: 'Failed to fetch advisor overview' });
  }
};
