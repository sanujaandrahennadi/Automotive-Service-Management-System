...
const CustomerPaymentManagement = () => {
  ...
  // Fallback calculator to mirror server logic when calculation object is missing
  const computeFinalPaymentFallback = (sc) => {
    const serviceCost = Number(sc?.finalCost?.totalAmount || 0);
    const profit = serviceCost * 0.8; // 80% profit
    const advisorFixed = 100;
    const staffFixed = 60;
    const costWithProfit = serviceCost + profit + advisorFixed + staffFixed;
    const tax = 0; // Tax removed
    const subtotal = costWithProfit;
    const loyaltyPct = Number(sc?.loyaltyDiscount?.percentage || 0);
    const loyaltyDiscountAmount = subtotal * (loyaltyPct / 100);
    const final = subtotal - loyaltyDiscountAmount;
    return {
      profit,
      advisorFixed,
      staffFixed,
    ...
    const filterAndSortServiceCosts = () => {
      ...
      if (searchTerm) {
        ...
        filtered = filtered.filter(sc => 
          sc.customerId?.name?.toLowerCase().startsWith(searchPrefix) ||
          sc.vehiclePlate?.toLowerCase().startsWith(searchPrefix) ||
          sc.serviceType?.toLowerCase().startsWith(searchPrefix) ||
          sc.advisorId?.name?.toLowerCase().startsWith(searchPrefix)
        );
      }
          ...
          <Box display="flex" gap={2} alignItems="center" flexWrap="wrap" mb={3}>
            <div className="flex-1 min-w-[300px]">
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Search by prefix (e.g., 'M' for Michael, Mary...)"
                  value={searchTerm}
                      ...
                      <div>
                        <span className="font-bold text-green-400">
                          ${serviceCost.customerPaymentCalculation?.finalCustomerPayment?.toFixed(2) || 
                            computeFinalPaymentFallback(serviceCost).final.toFixed(2)}
                        </span>
                      </div>
                    </td>
                    <td className="p-4">
                      ...
                      <div className="text-center p-4 bg-slate-700/50 rounded-lg">
                        <Typography variant="h4" sx={{ color: '#4caf50', fontWeight: 700 }}>
                          ${selectedServiceCost.customerPaymentCalculation?.finalCustomerPayment?.toFixed(2) || 
                            computeFinalPaymentFallback(selectedServiceCost).final.toFixed(2)}
                        </Typography>
                        <Typography variant="body2" sx={{ color: '#a3b8d0' }}>
                          Final Payment
                        </Typography>
